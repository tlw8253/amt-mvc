//need to add http://localhost:3015 to Chrome security to alow cookies 
//  Privacy and security ==> Cookies and other site data ==> Sites that can always use cookies ==> Add
function onLoad(event) {
    fetch('http://localhost:3015/ers_current_user', {
        ...
        if (response.status === 401) {
            alert("test ers_current_user: 401");
            //window.location.href = '/index.html'
        } else if (response.status === 200) {
            alert("test ers_current_user: 200");
            return response.json();
        }
    }).then((user) => {
        return fetch(//`http://localhost:7000/user/${user.id}/ship`, {
            //`http://localhost:3015/ers_current_user`, {
            `http://localhost:3015/ers/1`, {
            'method': 'GET', 
            'credentials': 'include'
        });
    }).then((response) => {
